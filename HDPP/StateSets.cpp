...
StateSets::StateSets(Alignment* ap, Settings* sp) {
	...
	index           = alignmentPtr->getIndex();
	
	// allocate the state sets vector
	sts = new unsigned[3 * oneStateSetSize];
	if ( !sts )
		Msg::error("Problem allocating state sets");
...
int StateSets::initializeStateSets(std::vector<Node*>& dp) {
	...
	for (std::vector<Node*>::iterator p=dp.begin(); p != dp.end(); p++)
		{
	...
	// pass up
	for (std::vector<Node*>::reverse_iterator p=dp.rbegin(); p != dp.rend(); p++)
		{
		if ( (*p)->getAnc() != NULL )
			{
